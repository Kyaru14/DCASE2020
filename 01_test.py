...
if __name__ == "__main__":
    ...
    for idx, target_dir in enumerate(dirs):
        ...
        print("============== MODEL LOAD ==============")
        # set model path
        model_file = param['model_directory'] + f"{machine_type}.net"

        # load model file
        if not os.path.exists(model_file):
            com.logger.error("{} model not found ".format(machine_type))
            sys.exit(-1)
        model = ae_model.load_model(model_file)
        model.eval()
        ...
        for id_str in machine_id_list:
            ...
            for file_idx, file_path in tqdm(enumerate(test_files), total=len(test_files)):
                ...
                data = com.file_to_vector_array(file_path,
                                                n_mels=param["log_mel_spec"]["n_mels"],
                                                frames=param["log_mel_spec"]["frames"],
                                                n_fft=param["log_mel_spec"]["n_fft"],
                                                hop_length=param["log_mel_spec"]["hop_length"])
                data = datasets.Transform.to_tensor(data)
                errors = torch.nn.functional.mse_loss(model(data), data)
                y_pred[file_idx] = errors.detach().numpy()
